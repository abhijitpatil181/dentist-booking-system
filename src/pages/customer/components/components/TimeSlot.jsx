...
const TimeSlot = ({selectedSlot,onTimeSlotChange}) => {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center',justifyContent:'space-between'}}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', width: '70%',rowGap:'1rem',columnGap:'1rem' }}>
        {currentSlots.map((time, index) => (
          <div key={index} className="time-slot" onClick={()=>onTimeSlotChange(time)} style={{backgroundColor:selectedSlot===time ? ' #6c757d':'transparent'}}>
            <p>{time}</p>
          </div>
        ))}
      </div>
