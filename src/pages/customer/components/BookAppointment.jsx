...
const BookAppointment = ({
  ...
  useEffect(() => {
    if (selectedDentist.id && selectedService.id) {
      const serviceCost = servicesData.find(
  ...
  const serviceOption = servicesData.map(service => ({
        ...
        >
          ...
          {selectedDentist.id && selectedService.id && (
            <p style={{ fontSize: '1.2rem', fontWeight: 600 }}>
              Rs.{totalPrice}.00{' '}
              <span style={{ fontSize: '0.8rem', color: '#888', fontWeight: 400 }}>Including GST</span>
            </p>
          )}
        </div>
      </div>
