...
const CustomerDashboard = () => {
  ...
  return (
    ...
    <BookingContext.Provider value={{historyDate,setHistoryDate}}>
       ...
       {isBooking && <div style={{paddingTop:'1rem'}}>
        <BookingHistory/>
       </div> }
     </BookingContext.Provider>
      {isPaymentRoute && <Payment dentistId={selectedDentist.id} serviceId={selectedService.id} totalPrice={totalPrice} selectedSlot={selectedSlot} selectedDate={selectedDate} reset={reset}/>}
      {isCustomerDashboard && 
      <>
      <div style={{ display: 'flex', justifyContent: 'flex-end', height: '4vh', marginTop: '1rem' }}>
        <div style={{ border: '1px solid grey', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <button onClick={handlePrevMonth} style={{ marginLeft: '0.5rem', border: 'none', background: 'transparent', padding: '0.5rem', cursor: 'pointer', fontSize: '1.2rem' }}>
            &lt; {/* Previous month icon */}
          </button>
          <h6 style={{ fontSize: '1.2rem', margin: '0', fontWeight: 400 }}>{monthYearString}</h6> {/* Display current month and year */}
          <button onClick={handleNextMonth} style={{ marginLeft: '0.5rem', border: 'none', background: 'transparent', padding: '0.5rem', cursor: 'pointer', fontSize: '1.2rem' }}>
            &gt; {/* Next month icon */}
          </button>
        </div>
        {logInDetails.role==="customer" &&<div style={{ border: '1px solid grey', display: 'flex', alignItems: 'center', width: '12.6vw', justifyContent: 'center' }}>
          <h6 style={{ fontSize: '1.2rem',fontWeight:400,cursor:'pointer' }} onClick={()=>{
            const formattedDate = format(new Date(), 'yyyy-MM-dd');
            setHistoryDate(formattedDate);
            navigate('booking-history')}}>Booking History</h6>
        </div>}
      </div>
      ...
      />
        ...
        content={<BookAppointment date={selectedDate} dentistData={dentistData} servicesData={servicesData} selectedDentist={selectedDentist} selectedService={selectedService} totalPrice={totalPrice} setTotalPrice={setTotalPrice} selectedSlot={selectedSlot} setSelectedDentist={setSelectedDentist} setSelectedService={setSelectedService} setSelectedSlot={setSelectedSlot} />} 
