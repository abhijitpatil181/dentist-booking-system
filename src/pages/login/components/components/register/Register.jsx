...
const Register=({formType=""})=>{
  ...
  const [isChecked, setIsChecked] = useState(false);
   ...
   const onChangeHandler = (label, value) => {
		...
		const registerUser=async (username,password,fullname,email,mobileNumber,gender,age=10,servicesRequired=[],hourlyRate=100)=>{
			try {
		...
		const onSubmitHandler=(e)=>{
			...
			switch(role){
				...
				case "customer":
					registerUser(username,password,fullname,email,mobileNumber,gender,age,servicesRequired.split(','))
					break;
				...
				<div className="form-container">
					...
					<div className="input-container">
						...
						{formType==="register" && <TextField
							...
							style={{ width: 'calc(50% - 1rem)' }}
						/>}
						{formType==="add-dentist" && <TextField
							label="Hourly Rate"
							value={hourlyRate}
					...
					<div className="input-container">
						...
						{formType==="register" && <TextField
							...
							onChange={e =>
								onChangeHandler('servicesRequired', e.target.value)
							}
							required={true}
							type="text"
							style={{ width: 'calc(50% - 1rem)' }}
					...
					<div className="input-container" style={{width:formType==="register" ? '100%':"50%"}}>
						{formType==="register" && <div style={{ display: 'flex', alignItems: 'center' }}>
							<label>
								<input
									type="checkbox"
