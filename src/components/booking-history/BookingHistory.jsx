...
const timeSlots = [
  ...
  '3:00 PM', '4:00 PM', '5:00 PM', '5:30 PM'
];
...
const BookingHistory = () => {
  ...
  const calculateWeekDates = (startOfWeekDate) => {
    const dates = Array.from({ length: 6 }, (_, i) => format(add(startOfWeekDate, { days: i }), 'yyyy-MM-dd'));
    setWeekDates(dates);
  ...
  const getBookingsForDay = (date) => {
    return data.filter((booking) => {
      ...
      return formattedDate === date && (booking.customerId===logInDetails.username || booking.dentistId===logInDetails.username);
    });
  };
  ...
  const handleSlotClick=(booking,slot)=>{
    if(booking.bookingId && slot && logInDetails.role==="dentist"){  
      ...
      setSlotData({patientName,serviceName,serviceId:bookedServiceId,bookingId:booking.bookingId,slot});
      setOpen(true);
    } 
    ...
    try{
       const bookingsRef = collection(db, 'bookings');
        ...
        const customerQuery = query(
            bookingsRef,
            where('bookingId', '==', slotData.bookingId),   
        );
        ...
        querySnapshot.forEach(async (docSnapshot) => {
          ...
          const updatedServicesData = bookingData.serviceData.map(service => {
            if (service.slot === slotData.slot) {  // Match slot if you have a specific slot to match
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', height: '2.5rem' }}>
        <p style={{ fontSize: '1.5rem', fontWeight: 400, paddingBottom: '0.3rem', paddingLeft: '0.3rem' }}>Booking History</p>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          {bookingHistory && <button onClick={handlePreviousWeek} style={{ marginLeft: '0.5rem', border: 'none', background: 'transparent', padding: '0.5rem', cursor: 'pointer', fontSize: '1.2rem' }}>
            &lt;
          </button>}
          <h6 style={{ fontSize: '1.2rem', margin: '0', fontWeight: 400 }}>{`${startDate} - ${endDate}`}</h6>
          {bookingHistory && <button onClick={handleNextWeek} style={{ marginLeft: '0.5rem', border: 'none', background: 'transparent', padding: '0.5rem', cursor: 'pointer', fontSize: '1.2rem' }}>
            &gt;
          </button>}
        </div>
        <button
        ...
        <thead>
          <tr>
            ...
            {daysOfWeek.map((day, index) => (
              <th key={index} style={{ border: '1px solid #D9D9D9', padding: '8px' }}>{day}</th>
            ))}
          </tr>
        </thead>
        <tbody>
          {timeSlots.map((time, index) => (
            <tr key={index} style={{ position: 'relative' }}>
              ...
              {weekDates.map((date, dayIndex) => {
                ...
                return (
                  <td key={dayIndex} style={{ border: '1px solid #D9D9D9', padding: '0.5rem', position: 'relative', height: '4rem' }}>
                    {bookingForTimeSlot && bookingForTimeSlot.serviceData.some((bookedSlot) => bookedSlot.slot === time) ? (
                      <BookingBlock key={bookingForTimeSlot.bookingId} booking={bookingForTimeSlot} dentistData={dentistData.find((dentist) => dentist.username === bookingForTimeSlot.dentistId)} handleSlotClick={handleSlotClick} timeSlot={time}/>
                    ) : null}
                  </td>
                );
              })}
